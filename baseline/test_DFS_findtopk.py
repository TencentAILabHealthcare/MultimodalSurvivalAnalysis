...
def get_args(parser):
    ...
    parser.add_argument("--name", type=str, default="DFS_SEED0")
    parser.add_argument("--savedir", type=str, default="/data3/louwei/MedComm/codes/baseline/checkpoints/save_dir")
...
def model_eval_with_attention(test_loader, model, logger, top_k=5):
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm(test_loader, desc="Testing")):
            ...
            OS_all.append(batch['DFS'].float().cpu().numpy())
            OSEvent_all.append(batch['DEvent'].float().cpu().numpy())
...
def main():
    ...
    output_path = os.path.join(savedir, "predictions_with_attention.xlsx")
