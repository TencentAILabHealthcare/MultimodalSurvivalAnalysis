...
def model_eval(test_loader, model, dataname):
    ...
    # cindex_test = CIndex_lifeline(pred_all, OSEvent_all, OS_all)
    # pvalue_test = cox_log_rank(pred_all, OSEvent_all, OS_all)
    # surv_acc_test = accuracy_cox(pred_all, OSEvent_all)

    # metrics = {"loss": np.mean(loss.item())}
    # metrics["cindex"] = cindex_test
    # metrics["pvalue"] = pvalue_test
    # metrics["survacc"] = surv_acc_test
    return metrics
...
if __name__ == '__main__':
    ...
    for i_epoch in range(start_epoch, opt.max_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            #print(i,pred)
            if i != 0:
                OS_all = torch.cat((OS_all, batch['OS'].cuda(range(torch.cuda.device_count())[0]).float()), 0)
                OSEvent_all = torch.cat((OSEvent_all, batch['OSEvent'].cuda(range(torch.cuda.device_count())[0]).float()), 0)                
                pred_all = torch.cat((pred_all, pred), 0)
            else:
                pred_all = pred
                OS_all, OSEvent_all = batch['OS'].cuda(range(torch.cuda.device_count())[0]).float(), batch['OSEvent'].cuda(range(torch.cuda.device_count())[0]).float()
