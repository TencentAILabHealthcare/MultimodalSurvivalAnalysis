...
def model_eval(test_loader, model, logger):
    ...
    with torch.no_grad():
        for i, batch in enumerate(tqdm(test_loader, desc="Testing")):
            ...
            if i == 0:
                pred_all = pred
                OS_all = batch['OS'].float().cpu()
                OSEvent_all = batch['OSEvent'].float().cpu()
            else:
                pred_all = torch.cat((pred_all, pred), dim=0)
                OS_all = torch.cat((OS_all, batch['OS'].float().cpu()), dim=0)
                OSEvent_all = torch.cat((OSEvent_all, batch['OSEvent'].float().cpu()), dim=0)
...
def main():
    ...
    pred_all, OS_all, OSEvent_all = model_eval(test_loader, model, logger)
    # 4. 保存预测结果
    # 保存为 Excel 文件
    results = {
        "ID": test_fp['ID'],
